...
def data_preprocessing(task_1a_dataframe):
	...
	minority_class = task_1a_dataframe[task_1a_dataframe['LeaveOrNot'] == 1]  # Assuming 'target' is your class column
	majority_class = task_1a_dataframe[task_1a_dataframe['LeaveOrNot'] == 0]

	# Calculate the imbalance ratio
	imbalance_ratio = len(majority_class) // len(minority_class)

	# Duplicate instances of the minority class
	duplicated_minority_class = pd.concat([minority_class] * imbalance_ratio, ignore_index=True)

	# Combine the duplicated minority class with the majority class
	task_1a_dataframe = pd.concat([majority_class, duplicated_minority_class], ignore_index=True)
	task_1a_dataframe = task_1a_dataframe.sample(frac=1).reset_index(drop=True)
	columns_to_encode = ['Education', 'City', 'Gender', 'EverBenched']
	encoded_dataframe = pd.get_dummies(task_1a_dataframe, columns=columns_to_encode)
...
def identify_features_and_targets(encoded_dataframe):
    ...
    # Select all columns that start with the prefixes of the encoded columns
    encoded_columns = [col for col in encoded_dataframe.columns if col.startswith(('Education', 'City', 'Gender', 'EverBenched'))]
    other_columns = ['JoiningYear', 'PaymentTier', 'Age', 'ExperienceInCurrentDomain']
    x_columns = encoded_columns + other_columns
    x = encoded_dataframe[x_columns]
...
class Salary_Predictor(nn.Module):
  ...
  def __init__(self):
        ...
        self.fc1 = nn.Linear(14, 64)  # First hidden layer with 64 units
