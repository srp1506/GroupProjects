...
def data_preprocessing(task_1a_dataframe):
	...
	numerical_columns = ['JoiningYear', 'Age','ExperienceInCurrentDomain']  # Add your numerical columns here
	...
	label_encoder_columns = ['Education','City','Gender','EverBenched']  # Add columns you want to label encode here
...
def identify_features_and_targets(encoded_dataframe):
    ...
    x_columns = ['JoiningYear','City', 'Age','ExperienceInCurrentDomain','Education', 'PaymentTier', 'Gender', 'EverBenched']
...
def load_as_tensors(features_and_targets):
  ...
  input_shape = 8  # Number of features expected by the model's first layer
  X_train_reshaped = X_train.view(-1, input_shape)
  X_test_reshaped = X_test.view(-1, input_shape)
...
class Salary_Predictor(nn.Module):
  ...
  def __init__(self):
        ...
        self.fc1 = nn.Linear(8, 4)  # First hidden layer with 64 units
        self.fc2 = nn.Linear(4, 2)  # First hidden layer with 64 units
       	#self.fc3 = nn.Linear(7, 4)
        self.output = nn.Linear(2, 1)  # Output layer with 1 unit
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
  ...
  for epoch in range(num_epochs):
      for batch_X, batch_y in train_loader:
          ...
          loss = loss_function(outputs.squeeze(), batch_y.squeeze())
          optimizer.zero_grad()
          loss.backward()
          optimizer.step()
