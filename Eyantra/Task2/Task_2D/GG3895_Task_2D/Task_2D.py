...
# Team ID:			[ GG_3895 ]
# Author List:		[ Soham Rajesh Pawar, Siddhant Sanjay Godbole, Aditya Abhaykumar Waghmare, Ashwin Agrawal ]
# Filename:			Task_2D.py
###################################################################################################
...
def read_csv(csv_name):
    ...
    with open(csv_name, 'r') as file:
        ...
        for row in reader:
            ...
            column2 = row[1]
            column3 = row[2]
            lat_lon[column1] = [column2, column3]
...
def tracker(ar_id, lat_lon):
    ...
    data = lat_lon[str(ar_id)]
    write_csv(data, 'live_data.csv')
    coordinate = [float(data[0]), float(data[1])]
